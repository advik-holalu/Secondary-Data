...
# Month normalization (same as dashboard)
month_map = {
    "january": 1, "jan": 1,
    "february": 2, "feb": 2,
    "march": 3, "mar": 3,
...
valid_months = [4, 5, 6, 7, 8, 9, 10]   # Apr→Oct only
...
# Month → MonthKey → MonthNum
df["MonthKey"] = df["Month"].str[:3].str.lower()
df["MonthNum"] = df["MonthKey"].map(month_map)

# Drop anything not Apr–Oct
df = df[df["MonthNum"].isin(valid_months)]

# Make MonthLabel
df["MonthLabel"] = df["MonthNum"].map(month_label_map)

# Final sort
df = df.sort_values(["Primary Cat", "Platform", "City Name", "MonthNum"]).reset_index(drop=True)

# ---------------------------
# SAVE
# ---------------------------
df.to_parquet(TARGET_FILE, index=False)
print(f"✅ Saved industry size parquet → {TARGET_FILE} (shape={df.shape})")
